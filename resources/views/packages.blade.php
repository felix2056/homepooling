...
<main id="content" class="share_room">
	...
	<div class="container packages_wrapper">
		@if(isset($current)&&$current=='basic')
			<form id="package_form" action="/properties/{{$property->id}}/packages" method="POST">
				{!! csrf_field() !!}
		...
		@else
			<form id="package_form" action="/properties/{{$property->id}}/packages" method="POST">
				{!! csrf_field() !!}
				<input id="choose_basic" type="radio" name="package" value="basic" selected checked @if(isset($current)&& $current) @if($current=='premium') disabled @endif @endif/>
				<label class="choice @if(isset($current)&& $current) @if($current=='premium') disabled @endif @endif" for="choose_basic">
					<div id="basic" class="package">
						<div class="package_row first">
							<h3>Basic</h3>
							<h1>Free</h1>
							<h5>Free of charge standard listing</h5>
							<h5>Seekers can't contact you during the first week</h5>
						</div>
						<div class="package_row">
							<h5>No Highlights</h5>
							<h5>No Early Access</h5>
							<h5>Includes 3 free conversations with seekers</h5>
				...
				<label class="choice @if(isset($current)&&$current=='premium') disabled @endif" for="choose_premium">
					<div id="premium" class="package">
						<div class="package_row first">
							...
							<h5>Seekers can contact you right away!</h5>
						</div>
						<div class="package_row">
							<h5>On Top for a Week</h5>
							<h5>Early Access</h5>
							<h5>Unlimited conversations</h5>
							<h5>Allow early conversation from all or premium seekers</h5>
						</div>

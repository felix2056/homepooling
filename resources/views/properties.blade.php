@section('pageTitle')
Properties - Pool Your Home
@endsection

@extends('layouts.app')

@section('content')
<main id="content" class="search_list again">
		<div class="results-map-toggle-wrapper container">
			<div class="">
				<span class="col-xs-6 results-map-toggle active" data-toggle="results">Results</span>
				<span class="col-xs-6 results-map-toggle" data-toggle="map">Map</span>
			</div>
		</div>
		<div class="col-md-12 col-lg-12 col-sm-12 col-xs-12">
			<div id="scroller" class="list_wrapper show col-md-7 col-sm-12 col-xs-12">
				<div id="search_map">
					<div id="search_bar">
						<input type="hidden" id="lat" name="lat" value="{{ Request::get('lat') }}">
						<input type="hidden" id="long" name="long" value="{{ Request::get('long') }}">
						<div class="pd5">
							<input id="autocomplete" name="location" class="location" @if(Request::has('location')) value="{{ Request::get('location') }}" @elseif(Auth::check() && isset(Auth::user()->preferences) && isset(Auth::user()->preferences->location)) value="{{Auth::user()->location}}" @endif />
						</div>
						...
						<div>
							<button class="btn-more pull-left">More <i class="fa fa-caret-down"></i></button>
						</div>
						<div>
						<button class="reset_form pull-left">Reset Form</button>
						</div>
					</div>
					<div id="search_bar" class="more">
				...
				<h2 class="props">Rooms matching your search</h2>
				<div class="listings-grid props"></div>
			</div>
			<div class="gmaps_wrapper col-md-5 col-sm-12 col-xs-12">
				<div id="gmaps"></div>
			</div>
		</div>
</main>
...
@section('script')
    <?php //<script src="{{ asset('js/search.js') }}"></script> ?>
@endsection
